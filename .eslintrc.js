module.exports = {
    ...
    rules: {
        ...
        // 'no-compare-neg-zero': 2, // [r] 禁止与 -0 进行比较
        // 'no-cond-assign': 2, // [r] 禁止条件表达式中出现赋值操作符
        'no-console': 0, // [r] 禁用 console
        // 'no-constant-condition': 2, // [r] 禁止在条件中使用常量表达式
        // 'no-control-regex': 2, // [r] 禁止在正则表达式中使用控制字符
        // 'no-debugger': 2, // [r] [f] 禁用 debugger
        // 'no-dupe-args': 2, // [r] 禁止 function 定义中出现重名参数
        // 'no-dupe-keys': 2, // [r] 禁止对象字面量中出现重复的 key
        // 'no-duplicate-case': 2, // [r] 禁止出现重复的 case 标签
        // 'no-empty': 2, // [r] 禁止出现空语句块
        // 'no-empty-character-class': 2, // [r] 禁止在正则表达式中使用空字符集
        // 'no-ex-assign': 2, // [r] 禁止对 catch 子句的参数重新赋值
        // 'no-extra-boolean-cast': 2, // [r] [f] 禁止不必要的布尔转换
        'no-extra-parens': [2, 'functions'], // [f] 禁止不必要的括号
        // 'no-extra-semi': 2, // [r] [f] 禁止不必要的分号
        // 'no-func-assign': 2, // [r] 禁止对 function 声明重新赋值
        'no-inner-declarations': [2, 'functions'], // [r] 禁止在嵌套的块中出现变量声明或 function 声明
        // 'no-invalid-regexp': 2, // [r] 禁止 RegExp 构造函数中存在无效的正则表达式字符串
        // 'no-irregular-whitespace': 2, // [r] 禁止在字符串和注释之外不规则的空白
        // 'no-obj-calls': 2, // [r] 禁止把全局对象作为函数调用
        // 'no-prototype-builtins': 0, // 禁止直接调用 Object.prototypes 的内置属性
        // 'no-regex-spaces': 2, // [r] [f] 禁止正则表达式字面量中出现多个空格
        // 'no-sparse-arrays': 2, // [r] 禁用稀疏数组
        // 'no-template-curly-in-string': 0, // 禁止在常规字符串中出现模板字面量占位符语法
        // 'no-unexpected-multiline': 2, // [r] 禁止出现令人困惑的多行表达式
        // 'no-unreachable': 2, // [r] 禁止在return、throw、continue 和 break 语句之后出现不可达代码
        // 'no-unsafe-finally': 2, // [r] 禁止在 finally 语句块中出现控制流语句
        // 'no-unsafe-negation': 2, // [r] [f] 禁止对关系运算符的左操作数使用否定操作符
        // 'use-isnan': 2, // [r] 要求使用 isNaN() 检查 NaN
        ...
        'curly': [2, 'multi-line'], // [f] 强制所有控制语句使用一致的括号风格
        'default-case': 2, // 要求 switch 语句中有 default 分支
        'dot-location': [2, 'property'], // [f] 强制在点号之前和之后一致的换行
        'dot-notation': 2, // [f] 强制尽可能地使用点号
        'eqeqeq': [2, 'allow-null'], // [f] 要求使用 === 和 !==
        ...
        // 'no-else-return': 0, // [f] 禁止 if 语句中 return 语句之后有 else 块
        // 'no-empty-function': 0, // 禁止出现空函数
        // 'no-empty-pattern': 2, // [r] 禁止使用空解构模式
        // 'no-eq-null': 0, // 禁止在没有类型检查操作符的情况下与 null 进行比较
        // 'no-eval': 0, // 禁用 eval()
        'no-extend-native': 2, // 禁止扩展原生类型
        // 'no-extra-bind': 0, // [f] 禁止不必要的 .bind() 调用
        // 'no-extra-label': 0, // [f] 禁用不必要的标签
        // 'no-fallthrough': 2, // [r] 禁止 case 语句落空
        // 'no-floating-decimal': 0, // [f] 禁止数字字面量中使用前导和末尾小数点
        // 'no-global-assign': 2, // [r] 禁止对原生对象或只读的全局对象进行赋值
        // 'no-implicit-coercion': 0, // [f] 禁止使用短符号进行类型转换
        ...
        // 'array-bracket-newline': 0, // [f] 在数组开括号后和闭括号前强制换行
        ...
        // 'no-lonely-if': 0, // [f] 禁止 if 作为唯一的语句出现在 else 语句中
